...
<div class="min-h-[100svh] bg-white dark:bg-gray-950 mt-5">
    <div class="mx-auto w-full max-w-md px-5 pb-8 pt-10 sm:px-6 sm:pt-12" x-data="{ show: false, step: {{ $errors->has('password') || $errors->has('password_confirmation') ? 2 : 1 }} }" x-init="setTimeout(() => show = true, 80)">
        <div x-show="show" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
            ...
            <h1 class="mt-1 text-3xl font-black tracking-tight text-gray-900 dark:text-white outfit">Join now</h1>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Set up your account in two quick steps.</p>

            <div class="mt-5 flex items-center gap-3">
                <div class="flex items-center gap-2">
                    <div :class="step >= 1 ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-800 dark:text-gray-400'" class="flex h-8 w-8 items-center justify-center rounded-full text-xs font-black">1</div>
                    <span class="text-[11px] font-black uppercase tracking-widest" :class="step >= 1 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400'">Profile</span>
                </div>
                <div class="h-px flex-1" :class="step >= 2 ? 'bg-emerald-500' : 'bg-gray-200 dark:bg-gray-800'"></div>
                <div class="flex items-center gap-2">
                    <div :class="step >= 2 ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-800 dark:text-gray-400'" class="flex h-8 w-8 items-center justify-center rounded-full text-xs font-black">2</div>
                    <span class="text-[11px] font-black uppercase tracking-widest" :class="step >= 2 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400'">Security</span>
                </div>
            </div>
            ...
            @if ($errors->any())
                <ul class="mt-4 space-y-1">
                    @foreach ($errors->all() as $error)
                        <li class="text-sm font-semibold text-red-600 dark:text-red-400">{{ $error }}</li>
                    @endforeach
                </ul>
            @endif

            <div class="mt-8 space-y-2.5 mb-4">
                <a href="{{ route('social.redirect', 'google') }}" class="relative flex h-12 w-full items-center justify-center rounded-2xl border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:hover:bg-gray-800">
                ...
                <a href="{{ route('social.redirect', 'facebook') }}" class="relative flex h-12 w-full items-center justify-center rounded-2xl bg-[#1877F2] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#166FE5]">
                    <span class="absolute left-4 flex h-5 w-5 items-center justify-center" aria-hidden="true">
                        <svg class="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="white" d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.099 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.412c0-3.026 1.792-4.697 4.533-4.697 1.312 0 2.684.235 2.684.235v2.98h-1.513c-1.49 0-1.955.93-1.955 1.884v2.26h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.099 24 12.073Z"/>
                        </svg>
                    </span>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-3.5" x-data="{ submitting: false }" @submit="submitting = true">
                ...
                <div x-show="step === 1" class="space-y-3.5" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                    ...
                    <button type="button" @click="step = 2" class="mt-2 h-12 w-full rounded-2xl bg-emerald-500 text-base font-black text-white transition hover:bg-emerald-400">
                        Continue
                    </button>
                </div>

                <div x-show="step === 2" class="space-y-3.5 " x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                    <div x-data="{ showPassword: false }">
                        <div class="relative mb-4">
                            <input id="password" name="password" :type="showPassword ? 'text' : 'password'" required placeholder=" "
                                class="peer h-12 w-full rounded-2xl border border-gray-300 bg-white px-4 pr-11 pt-5 text-sm font-medium text-gray-900 placeholder:text-transparent outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                            <label for="password" class="pointer-events-none absolute left-4 top-2 bg-white px-1 text-xs font-semibold text-gray-500 transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:bg-transparent peer-placeholder-shown:px-0 peer-placeholder-shown:text-sm peer-placeholder-shown:font-medium peer-focus:top-2 peer-focus:translate-y-0 peer-focus:bg-white peer-focus:px-1 peer-focus:text-xs peer-focus:font-semibold peer-focus:text-emerald-600 dark:bg-gray-900 dark:text-gray-400 dark:peer-placeholder-shown:bg-transparent dark:peer-focus:bg-gray-900 dark:peer-focus:text-emerald-400">
                                Password
                            </label>
                        ...
                        @error('password')
                            <p class="mt-1.5 text-xs font-semibold text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="relative">
                        <input id="password_confirmation" name="password_confirmation" type="password" required placeholder=" "
                            class="peer h-12 w-full rounded-2xl border border-gray-300 bg-white px-4 pt-5 text-sm font-medium text-gray-900 placeholder:text-transparent outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100"
                            >
                        <label for="password_confirmation" class="pointer-events-none absolute left-4 top-2 bg-white px-1 text-xs font-semibold text-gray-500 transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:bg-transparent peer-placeholder-shown:px-0 peer-placeholder-shown:text-sm peer-placeholder-shown:font-medium peer-focus:top-2 peer-focus:translate-y-0 peer-focus:bg-white peer-focus:px-1 peer-focus:text-xs peer-focus:font-semibold peer-focus:text-emerald-600 dark:bg-gray-900 dark:text-gray-400 dark:peer-placeholder-shown:bg-transparent dark:peer-focus:bg-gray-900 dark:peer-focus:text-emerald-400">
                            Confirm Password
                        </label>
            ...
            <p class="mt-5 text-center text-sm text-gray-500 dark:text-gray-400">
                ...
                <a href="{{ route('login') }}" class="font-bold text-emerald-600 dark:text-emerald-400">Sign in</a>
            </p>
        </div>
    </div>
</div>
