        ...
        window.sendOrderNotification = function (orderId, newStatus, message) {
            ...
            if (navigator.serviceWorker?.controller) {
                navigator.serviceWorker.controller.postMessage({
                    ...
                    body: body,
                    tag: `order-${orderId}`,
                    url: `/order/${orderId}/confirmation`,
                });
                ...
                window.__swRegistration.showNotification(title, {
                    ...
                    tag: `order-${orderId}`,
                    renotify: true,
                    data: { url: `/order/${orderId}/confirmation` },
                    vibrate: [200, 100, 200],
                });
            }
        };
...
<body
    ...
    <div id="page-loader"
        class="fixed top-20 inset-x-0 bottom-0 bg-white dark:bg-gray-900 z-40 overflow-hidden pointer-events-none transition-opacity duration-300">
        <!-- Body Skeleton Grid -->
        @hasSection('skeleton')
            @yield('skeleton')
        @else
            <div class="max-w-7xl mx-auto px-4 mt-8 md:mt-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
                    @for ($i = 0; $i < 6; $i++)
                        <div
                            class="h-[300px] bg-gray-200 dark:bg-gray-800 rounded-[2rem] animate-pulse @if($i > 1) hidden md:block @endif @if($i > 2) hidden lg:block @endif">
                        </div>
                    @endfor
                </div>
            </div>
    ...
    <nav x-data="{ mobileMenuOpen: false }"
        ...
        class="bg-white/80 dark:bg-gray-900/95 backdrop-blur-xl sticky top-0 z-50 border-b border-gray-100 dark:border-gray-800 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 md:h-20">
                <div class="flex items-center">
                    <a href="{{ url('/') }}" class="hidden md:flex items-center group gap-1.5">
                        <div
                            class="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center transform group-hover:scale-105 transition-all duration-300">
                            <img src="{{ asset('images/brand/ansareats-logo-v2.svg') }}" alt="AnsarEats logo"
                                class="w-full h-full" width="56" height="56">
                        </div>
                        <span
                            class="font-extrabold text-lg md:text-2xl outfit text-gray-900 dark:text-white group-hover:text-emerald-500 tracking-tight transition-colors">AnsarEats</span>
                    </a>
                    ...
                    @auth
                        ...
                        <div x-data="{ open: false }" class="relative flex md:hidden items-center">
                            <button @click="open = !open" @click.away="open = false"
                                class="flex items-center gap-2 group text-left outline-none">
                                ...
                                <div class="flex flex-col justify-center">
                                    <span
                                        class="text-[10px] font-bold text-gray-400 tracking-wider leading-none mb-1">Delivering
                                        to</span>
                                    <div class="flex items-center gap-1">
                                        <span
                                            class="text-sm font-black text-gray-900 dark:text-white leading-none max-w-[120px] truncate">{{ $defaultLocation ? $defaultLocation->alias : 'Select Location' }}</span>
                            ...
                            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                                ...
                                <div class="max-h-[50vh] overflow-y-auto overscroll-contain">
                                    @if($userLocations->isNotEmpty())
                                        @foreach($userLocations as $loc)
                                            <form action="{{ route('profile.locations.default', $loc) }}" method="POST"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="w-full text-left px-4 py-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50 {{ $defaultLocation && $defaultLocation->id === $loc->id ? 'bg-emerald-50/50 dark:bg-emerald-500/5' : '' }}">
                                                    <div class="flex items-start gap-3">
                                                        <svg class="w-4 h-4 mt-0.5 flex-shrink-0 {{ $defaultLocation && $defaultLocation->id === $loc->id ? 'text-emerald-500' : 'text-gray-400' }}"
                                                        ...
                                                        <div class="flex-1 min-w-0">
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                @if($defaultLocation && $defaultLocation->id === $loc->id)
                                                                    <span
                                                                        class="inline-flex px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-700">Default</span>
                                                                @endif
                                                            </div>
                                                            <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                                                                {{ $loc->address ?: 'Coordinates: ' . $loc->latitude . ', ' . $loc->longitude }}
                                                            </p>
                                                        </div>
                                                    </div>
                                                </button>
                                    ...
                                    @else
                                        <div class="p-4 text-center">
                                            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">No locations saved
                                                yet.</p>
                                        </div>
                                    @endif
                                </div>
                                <div class="p-2 bg-gray-50 dark:bg-gray-800/50">
                                    <a href="{{ route('profile.locations') }}"
                                        class="block w-full py-1 text-center text-xs font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
                                        Manage Locations
                                    </a>
                                </div>
                            </div>
                    ...
                    @else
                        <a href="{{ url('/') }}" class="flex md:hidden items-center group gap-1.5">
                            <div
                                class="w-10 h-10 flex items-center justify-center transform group-hover:scale-105 transition-all duration-300">
                                <img src="{{ asset('images/brand/ansareats-logo-v2.svg') }}" alt="AnsarEats logo"
                                    class="w-full h-full" width="56" height="56">
                            </div>
                            <span
                                class="font-extrabold text-lg outfit text-gray-900 dark:text-white group-hover:text-emerald-500 tracking-tight transition-colors">AnsarEats</span>
                        </a>
                    @endauth

                    <div class="hidden lg:flex items-center ml-10 space-x-8">
                        <a href="{{ route('restaurants.index') }}"
                            class="text-sm font-bold text-gray-600 dark:text-gray-400 hover:text-emerald-500 transition-colors uppercase tracking-widest">Explore</a>
                        @if(!$hasRestaurant)
                            <a href="{{ route('partner.with.us') }}"
                                class="text-sm font-bold text-gray-600 dark:text-gray-400 hover:text-emerald-500 transition-colors uppercase tracking-widest">Partner
                                with us</a>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-2 md:space-x-4">
                    ...
                    <div x-data="{ 
                         ...
                         }" @click.away="expanded = false; show = false" class="relative hidden md:flex items-center">
                        ...
                        <div class="flex items-center bg-gray-100 dark:bg-gray-800 rounded-2xl transition-all duration-500 ease-in-out overflow-hidden h-11"
                            ...
                            <svg class="w-5 h-5 flex-shrink-0 transition-colors"
                                :class="expanded ? 'text-emerald-500' : 'text-gray-500'" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>

                            <input x-ref="navSearchInput" x-model="query" x-show="expanded"
                                x-transition:enter="transition ease-out duration-300 delay-100"
                                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                                @input.debounce.300ms="fetchSuggestions()" @focus="if(query.length >= 2) show = true"
                        ...
                        <div x-show="show && expanded && (results.restaurants.length > 0 || results.meals.length > 0)"
                            ...
                            <div class="h-80 md:h-96 overflow-y-auto overscroll-contain no-scrollbar pb-4"
                                style="-webkit-overflow-scrolling: touch;">
                                <template x-if="results.restaurants.length > 0">
                                    <div class="p-2">
                                        <div
                                            class="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-gray-400">
                                            Restaurants</div>
                                        <template x-for="r in results.restaurants" :key="r.id">
                                            <a :href="r.url"
                                                class="flex items-center gap-3 p-2 rounded-2xl hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-colors group">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-700 flex-shrink-0 overflow-hidden flex items-center justify-center">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="font-bold text-sm text-gray-900 dark:text-white truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors"
                                                        x-text="r.name"></div>
                                                    <div
                                                        class="text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase">
                                                        Restaurant</div>
                                                </div>
                                            </a>
                                        </template>
                                ...
                                <template x-if="results.meals.length > 0">
                                    <div class="p-2 border-t border-gray-50">
                                        ...
                                        <template x-for="m in results.meals" :key="m.id">
                                            <a :href="m.url"
                                                class="flex items-center gap-3 p-2 rounded-2xl hover:bg-emerald-50 transition-colors group">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-gray-100 flex-shrink-0 overflow-hidden flex items-center justify-center">
                                                    <template x-if="m.image">
                                                        <img :src="m.image" class="w-full h-full object-cover">
                                                    </template>
                                                    <template x-if="!m.image">
                                                        <div
                                                            class="bg-emerald-100 w-full h-full flex items-center justify-center text-emerald-500">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="font-bold text-sm text-gray-900 truncate group-hover:text-emerald-600 transition-colors"
                                                        x-text="m.name"></div>
                    ...
                    @unless(View::hasSection('hideCart'))
                        <div x-data="navCart" @cart-updated.window="updateFromEvent($event)">
                            <button @click="$dispatch('toggle-cart')"
                                class="relative font-semibold text-gray-600 dark:text-gray-400 hover:text-emerald-500 transition-colors p-2 rounded-xl hover:bg-emerald-50 dark:hover:bg-emerald-500/10 h-11 w-11 flex items-center justify-center">
                    ...
                    <div class="hidden md:flex items-center space-x-4">
                        @guest
                            <a href="{{ route('login') }}"
                                class="font-semibold text-gray-600 hover:text-emerald-500 transition-colors">Log In</a>
                            <a href="{{ route('register') }}"
                                class="bg-emerald-500 px-4 py-2 font-semibold text-white rounded-md transition-colors">Register</a>
                        ...
                        @else
                            <div class="relative" x-data="{ open: false }" @click.away="open = false">
                                <button @click="open = !open"
                                    ...
                                    <div
                                        class="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white text-xs font-black">
                                        {{ substr(auth()->user()->name, 0, 1) }}
                                    </div>
                                    <span
                                        class="font-bold text-gray-700 dark:text-gray-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{{ auth()->user()->name }}</span>
                                    <svg class="w-4 h-4 text-gray-400 group-hover:text-emerald-500 transition-all"
                                        :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                            d="M19 9l-7 7-7-7"></path>
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                    ...
                                    <a href="{{ route('profile.show') }}"
                                        class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-gray-600 dark:text-gray-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all group">
                                        <div
                                            class="w-8 h-8 rounded-xl bg-gray-50 dark:bg-gray-700 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-500/20 flex items-center justify-center text-gray-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                    ...
                                    <a href="{{ route('profile.orders') }}"
                                        class="flex items-center justify-between gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-gray-600 hover:bg-emerald-50 hover:text-emerald-600 transition-all group">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-xl bg-gray-50 group-hover:bg-emerald-100 flex items-center justify-center text-gray-400 group-hover:text-emerald-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @if(auth()->user()->role !== 'super_admin' && auth()->user()->restaurant)
                                        <a href="{{ route('owner.dashboard') }}"
                                            class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-all group">
                                            <div
                                                class="w-8 h-8 rounded-xl bg-gray-50 group-hover:bg-indigo-100 flex items-center justify-center text-gray-400 group-hover:text-indigo-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @elseif(auth()->user()->role !== 'super_admin')
                                        <a href="{{ route('partner.with.us') }}"
                                            class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-gray-600 hover:bg-emerald-50 hover:text-emerald-600 transition-all group">
                                            <div
                                                class="w-8 h-8 rounded-xl bg-gray-50 group-hover:bg-emerald-100 flex items-center justify-center text-gray-400 group-hover:text-emerald-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="px-4 py-3 flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400">
                                                <svg x-show="!$store.darkMode.on" class="w-4 h-4" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-red-500 hover:bg-red-50 transition-all group">
                                            <div
                                                class="w-8 h-8 rounded-xl bg-red-100/50 group-hover:bg-red-100 flex items-center justify-center text-red-400 group-hover:text-red-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <main class="flex-grow">
        @if(session('success'))
            <div x-data x-init="window.showAppToast({ message: '{{ addslashes(session('success')) }}', type: 'success' })">
            </div>
        @endif

        @if(session('error'))
            <div x-data x-init="window.showAppToast({ message: '{{ addslashes(session('error')) }}', type: 'error' })">
            </div>
        @endif
        ...
        window.initAlpineComponents = function () {
            ...
            if (!Alpine.data('cartDrawer')) {
                Alpine.data('cartDrawer', () => ({
                    ...
                    updateFromEvent(event) {
                        const newCart = event.detail || { items: {}, count: 0, total: 0 };
                        const oldCount = this.cart.count || 0;
                        const newCount = newCart.count || 0;
            ...
            if (!Alpine.data('mobileBottomNav')) {
                Alpine.data('mobileBottomNav', (initialTab) => ({
                    ...
                    navigateWithGlide(event, tab) {
                        const href = event.currentTarget?.href;
                        if (!href) return;

                        this.setPendingTab(tab);
